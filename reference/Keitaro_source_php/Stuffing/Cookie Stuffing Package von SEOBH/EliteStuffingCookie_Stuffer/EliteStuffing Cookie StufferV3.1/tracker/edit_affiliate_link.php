<?php
	...
	if (isset($_POST['affiliate']) AND isset($_POST['aff_link']) AND isset($_POST['aff_banner']))
	{
		...
		{
			$update = "UPDATE affiliate_links SET url = \"" . $_POST['aff_link'] . "\" WHERE id = \"" . $_GET['id'] . "\"";
			if (!mysql_query($update))
			{
				die('Error: ' . mysql_error());
			}	
			...
			{
				$update = "UPDATE affiliate_links SET banner = \"" . $_POST['aff_banner'] . "\" WHERE id = \"" . $_GET['id'] . "\"";
				if (!mysql_query($update))
				{
					die('Error: ' . mysql_error());
				}
				...
				{
					$update = "UPDATE affiliate_links SET affiliate_id = \"" . $_POST['affiliate'] . "\" WHERE id = \"" . $_GET['id'] . "\"";
					if (!mysql_query($update))
					{
						die('Error: ' . mysql_error());
					}
					...
					{
						$html = "<html><head><meta http-equiv=\"refresh\" content=1;url=\"index.php\"></head><body> Added Successfully, Redirecting in 1</body></html>";
					}
					echo $html;
				}
			}
	...
	{	
		...
		{
			if($row['affiliate_name'] == $affiliate)
			{
				$html .= "<option value=\"" . $row['id'] . "\" selected>" . $row['affiliate_name'] . "</option>";
			}
			else
			{
		...
		$html .= "</select><br />";
		$html .= "Please input your affiliate link: <input type=\"text\" size=50 maxlength=300 name=\"aff_link\" value=\"" . $url . "\"><br />";
		$html .= "Please input your affiliate banner: <input type=\"text\" size=50 maxlength=300 name=\"aff_banner\" value=\"" . $banner . "\"><br />";
